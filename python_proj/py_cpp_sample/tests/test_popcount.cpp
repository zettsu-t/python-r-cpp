...
namespace {
    ...
    using PyUint8Array = pybind11::array_t<uint8_t,
                                           pybind11::array::c_style |
                                           pybind11::array::forcecast>;

    using PyUint64Array = pybind11::array_t<uint64_t,
                                            pybind11::array::c_style |
                                            pybind11::array::forcecast>;
    ...
    const std::vector<uint64_t> Input_Uint64 {
        0, 1, 0xfe, 0xff, 0x100, 0x101, 0xfffe, 0xffff,
        0x10000, 0x10001, 0xfffffffeull, 0xffffffffull,
        0x100000000ull, 0x100000001ull,
        0x3c3c3c3c3c3c3c3cull, 0xc3c3c3c3c3c3c3c3ull,
        0xffffffffffffffffull
    };
    const std::vector<Count> Expected_Uint64 {
        0, 1, 7, 8, 1, 2, 15, 16, 1, 2, 31, 32, 1, 2, 32, 32, 64
    };

    template<typename T>
    auto create_numpy_data_type_pybind11() {
    ...
    bool check_numpy_array_type_pybind11(const ArrayType& actual, SizeType expected_size) {
        ...
        // Exact element type
        if (!actual.dtype().is(create_numpy_data_type_pybind11<ArrayElementType>())) {
            return false;
        }
    ...
    bool check_count_type_pybind11(const ArrayType& actual, SizeType expected_size) {
        return check_numpy_array_type_pybind11<Count>(actual, expected_size);
    }

    template<typename T>
    BoostDataType create_numpy_data_type_boost() {
    ...
    template<typename ArrayElementType, typename ArrayType, typename SizeType>
    bool check_numpy_array_type_boost(const ArrayType& actual, SizeType expected_size) {
        // 1-D array
        if (actual.get_nd() != 1) {
            return false;
        }
        ...
        // Exact element type
        if (actual.get_dtype() != create_numpy_data_type_boost<ArrayElementType>()) {
            return false;
        }
    ...
    std::vector<Count> setup_popcount(SizeType array_size, SizeType value_offset, Element* ptr) {
        ...
        for(decltype(array_size) index {0}; index<array_size; ++index) {
            ...
            for (size_t bit_index = 0; bit_index < (sizeof(low_value) * 8); ++bit_index) {
...
template <typename T>
class TestPopcountTyped : public ::testing::Test {};
using PopcountTypes = ::testing::Types<uint8_t, uint64_t>;
TYPED_TEST_CASE(TestPopcountTyped, PopcountTypes);
...
TYPED_TEST(TestPopcountTyped, ArraySizePybind11) {
    ...
    using PyArray = pybind11::array_t<Element,
                                      pybind11::array::c_style |
                                      pybind11::array::forcecast>;
...
TEST_F(TestPopcountPybind11, FullValuesUint8) {
    ...
    auto expected = setup_popcount<Element>(array_size, value_offset, arg_values);

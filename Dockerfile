...
RUN wget "https://boostorg.jfrog.io/artifactory/main/release/1.78.0/source/${BOOST_VERSION}.tar.gz" && tar xzf "${BOOST_VERSION}.tar.gz"
...
RUN make VERBOSE=1 && make test || ./test_popcount || exit 0

WORKDIR "${R_PROJECT_DIR}"
RUN rm -rf "${R_PROJECT_DIR}/tests/build" && mkdir -p "${R_PROJECT_DIR}/tests/build"
WORKDIR "${R_PROJECT_DIR}/tests/build"
RUN sed -i -e "s/sanitize=[a-z,]*/sanitize=memory -fno-omit-frame-pointer -fno-optimize-sibling-calls/" "${R_PROJECT_DIR}/tests/ClangOverrides.txt" && cp "${R_PROJECT_DIR}/tests/ClangOverrides.txt" "${R_PROJECT_DIR}/tests/build/"
RUN CXX=clang++ CC=clang cmake -DCMAKE_BUILD_TYPE=Debug -DCMAKE_USER_MAKE_RULES_OVERRIDE=ClangOverrides.txt ..
RUN make VERBOSE=1 && make test || ./test_popcount || exit 0
...
RUN python3.8 setup.py bdist_wheel && pip3 install --force --user dist/py_cpp_sample-0.0.1-cp38-cp38-linux_x86_64.whl && pytest tests
RUN find build -name "*.so" | xargs objdump -d -M intel | grep -i popcnt
RUN flake8 --exclude tests/build src tests setup.py && pylint src tests setup.py && mypy src

RUN pip3 install --force --user -e .
RUN export PYTHONPATH=src; pytest --cov=src --cov=tests --cov-report=html tests; unset PYTHONPATH
RUN coverage report -m && coverage html
...
RUN rm -rf "${PYTHON_PROJECT_DIR}/tests/build" && mkdir -p "${PYTHON_PROJECT_DIR}/tests/build"
WORKDIR "${PYTHON_PROJECT_DIR}/tests/build"
RUN sed -i -e "s/sanitize=[a-z,]*/sanitize=memory -fno-omit-frame-pointer -fno-optimize-sibling-calls/" "${PYTHON_PROJECT_DIR}/tests/ClangOverrides.txt" && cp "${PYTHON_PROJECT_DIR}/tests/ClangOverrides.txt" "${PYTHON_PROJECT_DIR}/tests/build/"
RUN CXX=clang++ CC=clang cmake -DCMAKE_BUILD_TYPE=Debug -DCMAKE_USER_MAKE_RULES_OVERRIDE=ClangOverrides.txt ..
RUN make VERBOSE=1 && make test || ./test_popcount || exit 0
